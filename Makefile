...
SERVICES := user-service seller-service product-service order-service notification-service media-service review-service api-gateway
...
migrate-up: ## Run all migrations (or: make migrate-up svc=user-service)
	@if [ -z "$(svc)" ]; then \
		for s in user-service seller-service product-service order-service notification-service media-service review-service; do \
			echo "\n▶ Migrating $$s..."; \
